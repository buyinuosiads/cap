    ...
    public partial class Product : UIPage
    {
        ...
        private void uiDataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == uiDataGridView1.Columns["编辑"].Index && e.RowIndex >= 0)
            {
                ...
                // 获取行数据
                string Column1 = row.Cells["ProductName"].Value.ToString(); // 替换YourColumnName为你需要的列名
                string Column2 = row.Cells["TechnologyName"].Value.ToString(); // 替换YourColumnName为你需要的列名
                string Column3 = row.Cells["Price"].Value.ToString(); // 替换YourColumnName为你需要的列名
